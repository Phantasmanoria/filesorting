...
完結に書くと**自動でファイルを指定した条件で再帰的に分別, ソートしてくれるソフト**.  
Perl版とRuby版があるが, Perl版をもう一度作り直したのがRuby版.  
...
同時にコードの視認性, コメント, オプションの改善, 動作の改善も行った.  
...
入力は`20KB`や`4MB`のように, **数字(1~999)と単位(B~PBまで)**のようにすること.  
...
このプログラムの一番の機能であるファイルの分類を実行する.  
詳細は[下記](#使い方 分類モード(初期設定時))を参照すること.  
...
指定を行うと, 指定された分割単位でファイルサイズ分類が行われる(例を挙げると, `20KB`とすると, 
`20KB`,`40KB`,`60KB`,...のようにファイルサイズ分類が行われる).  
この時, **指定された大きさを持っているディレクトリのうち, 最大の場所**に分類が行われる
(例を挙げると, 分割単位を`20KB`とすると, `49KB`のファイルは`40KB`以上だが`60KB`未満なので, `40KB`に分類が行われる).  
